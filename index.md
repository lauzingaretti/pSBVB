...
* Run Toy example 
```zcat GenosB.vcf.gz | perl vcf2tped2.pl -hap | cut -d ' ' -f 1,4- | / sbvb -i toy.par ```
 ```  mkdir -p results``` 
      ```  cp Y.grm.1  results``` 
       ``` cp Y.outy   results``` 
     ```   cp Y.outq   results``` 
...
**Citation**
L. Zingaretti, A. Monfort, M. Pérez-Enciso. 2018. Modeling and simulation tools for genomic selection in polyploid species: a case study in octoploid strawberry. Manuscript submitted for publication.
