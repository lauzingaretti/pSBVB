...
Note that Chrom and positions should be sorted and I1\_c1, I1\_c2,...,I1\_ch referes to the first individual genotype. For instance, if the organism is tetraploid, it could be 0 0 1 0, or 0 1 0 0, among others. To run: if you have a `.vcf` file (file could be compress vcf.gz) or a genotype format, i.e. the first two rows contain the chromosome and SNP position (in base pairs), and the following columns contain the genotypes (coded *0,1,…h*, where h is the ploidy level), you should run the following line:

``` r2
cat file.vcf | python3 vcftogen.py -p Ploidy | ~/Dropbox/DoctoradoCRAG/paper-1/article/software-help/src/sbvb  -i file.par
```

if you already have the standar input file (.gen) as we explained before, you only need to execute the following sentence:
